    ...
    <div class="card-body">
        ...
        @if($yearlyPlans->count() > 0)
            <div class="form-check">
                <input class="form-check-input" type="radio" name="period" id="periodMonthly" value="monthly" @if($currentRole->pivot->period != 'yearly') checked @endif>
                <label class="form-check-label" for="periodMonthly">
                    Monthly plans
                </label>
            </div>
            <div class="form-check mb-2">
                <input class="form-check-input" type="radio" name="period" id="periodYearly" value="yearly" @if($currentRole->pivot->period == 'yearly') checked @endif>
                <label class="form-check-label" for="periodYearly">
                    Yearly plans ({{ $yearlyPercentOff }}% off)
                </label>
            </div>
        @endif
        <div class="row" id="plans-monthly" @if($currentRole->pivot->period == 'yearly') style="display:none"@endif>
            @foreach($monthlyPlans as $plan)
                <div class="col">
                    ...
                    @if($currentPlan && $plan->stripe_plan_id == $currentPlan->stripe_plan)
                        ...
                        @endif
                    @else
                        <a href="#" class="btn btn-primary button-plan" data-period="monthly" data-plan-id="{{ $plan->id }}" data-plan-price="{{ config('saas.currency', '$') }}{{ number_format($plan->price / 100, 2) }}" data-plan-title="{{ $plan->title }}">{{ trans('global.billing.choose_this_plan') }}</a>
                    @endif
                </div>
            @endforeach
        </div>
        <div class="row" id="plans-yearly" @if($currentRole->pivot->period != 'yearly') style="display:none"@endif>
            @foreach($yearlyPlans as $plan)
                <div class="col">
                    <h2>{{ $plan->title }}</h2>
                    <h4>{{ config('saas.currency', '$') }}{{ number_format($plan->yearly_price / 100, 2) }} / year</h4>
                    @if($currentPlan && $plan->yearly_stripe_plan_id == $currentPlan->stripe_plan)
                        Your current plan.
                        @if(!$currentPlan->onGracePeriod())
                            <br /><br />
                            <a href="{{ route('admin.billing.cancel') }}" class="btn btn-danger" onclick="return confirm('Are you sure?')">Cancel plan</a>
                        ...
                        @endif
                    @else
                        <a href="#" class="btn btn-primary button-plan" data-period="yearly" data-plan-id="{{ $plan->id }}" data-plan-price="{{ config('saas.currency', '$') }}{{ number_format($plan->yearly_price / 100, 2) }}" data-plan-title="{{ $plan->title }}">{{ trans('global.billing.choose_this_plan') }}</a>
                    @endif
                </div>
            @endforeach
        </div>
            ...
            <form action="{{ route('admin.billing.checkout') }}" method="POST" id="checkout-form">
                ...
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <p id="card-error" class="mt-1"></p>

                        <button id="card-button" class="btn btn-primary mt-1">
                            Pay
                        </button>
...
<script>
    $(document).ready(function() {
        ...
        $('.button-plan').on('click', function (e) {
          ...
          $('#discount-status,#card-error').text('');
        ...
        $('input[name="period"]').on('change', function () {
          var unselected = $('input[name="period"][value!="' + this.value + '"]').val();
          $('#plans-' + this.value).show();
          $('#plans-' + unselected).hide();
        });
      });
</script>
@endsection
